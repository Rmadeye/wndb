...
from spaceship.models.base_ml import LogisticClassifier, RFClassifier
...
def train_model(data: DataProcessor, config=None):
    with wandb.init(
        project="spaceship", config=config  # projekt do którego będziemy logować
    ):  # nasz config yamla
...
if __name__ == "__main__":
    config = load_yaml_config(Path("wandb_sweep_2.yaml"))
